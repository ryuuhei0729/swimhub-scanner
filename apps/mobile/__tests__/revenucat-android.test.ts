 ...
 * - 未実装ケースは `it.todo` で宣言する (空 body の `it` は無条件 pass 扱いになり、
 *   実装漏れが CI 上で緑のまま見逃されるため)。
 *
...
describe("env.ts - revenuecatAndroidApiKey フィールド", () => {
  ...
  it.todo("should export revenuecatAndroidApiKey field");
  ...
  it.todo("should default to empty string when EXPO_PUBLIC_REVENUCAT_ANDROID_API_KEY is unset");
});
...
describe("revenucat.ts - Platform.OS !== 'ios' ガードが残っていないこと", () => {
  ...
  it.todo("should NOT have Platform.OS !== 'ios' guard in initRevenueCat (Android path reachable)");
});
...
describe("initRevenueCat - Android, 有効な goog_ キー", () => {
  ...
  it.todo("should call Purchases.configure with goog_ key on Android");

  // Given: 上記の初期化済み状態
  // When: initRevenueCat() を再度呼ぶ
  // Then: Purchases.configure の呼び出し回数が増えないこと
  it.todo("should NOT call Purchases.configure twice (idempotent)");
});
...
describe("initRevenueCat - Android, キー未設定", () => {
  ...
  it.todo("should NOT call Purchases.configure when Android key is empty string");

  // Given: Platform.OS = "android"
  //        env.revenuecatAndroidApiKey = "appl_wrong_prefix" (iOS のキーを誤設定)
  // When: initRevenueCat() を呼ぶ
  // Then: Purchases.configure が呼ばれないこと (prefix バリデーション)
  it.todo("should NOT call Purchases.configure when Android key does not start with goog_");
  ...
  it.todo("should NOT call Purchases.configure when Android key is placeholder");
});
...
describe("initRevenueCat - iOS, 既存パスの回帰確認", () => {
  ...
  it.todo("should call Purchases.configure with appl_ key on iOS");

  // Given: Platform.OS = "ios"
  //        env.revenuecatIosApiKey = "" (未設定)
  // When: initRevenueCat() を呼ぶ
  // Then: Purchases.configure が呼ばれないこと
  it.todo("should NOT call Purchases.configure on iOS when iOS key is empty");
  ...
  it.todo("should NOT call Purchases.configure on iOS with Android key even if set");
});
...
describe("revenucat 各関数 - 未初期化状態の no-op 確認", () => {
  ...
  it.todo("loginRevenueCat should no-op when not initialized");

  // Given: isInitialized = false
  // When: getOfferings() を呼ぶ
  // Then: null を返すこと
  it.todo("getOfferings should return null when not initialized");

  // Given: isInitialized = false
  // When: addCustomerInfoListener(jest.fn()) を呼ぶ
  // Then: 返り値が呼び出し可能な関数であること。呼び出してもエラーが発生しないこと
  it.todo("addCustomerInfoListener should return no-op cleanup when not initialized");
});
...
describe("account.tsx - manageSubscription URL Platform 分岐", () => {
  ...
  it.todo("should open apps.apple.com on iOS when manage subscription is pressed");
  ...
  it.todo("should open play.google.com on Android when manage subscription is pressed");
});
